...
export default class WaterHandler {
    ...
    constructor(scene: Phaser.Scene , mapManager: MapManager){
        ...
        scene.events.on(Events.AbsorbWater, (plantData: PlantData) => {
            ...
            waterStats.waterRemoved = this.removeWater(plantData);
            waterStats.waterAdded = this.addWaterFromWaterSources(plantData);
            this.updatePlayerWater(scene, plantData, waterStats);
            scene.events.emit(Events.RemoveWater, this.waterSources);
        })

        //TODO get rid of the whole body of water thing
        // scene.events.on(Events.AbsorbWater, (plantData: PlantData) => {
        //     let waterStats: waterStats = {totalWater: 0, waterAdded: 0, waterRemoved: 0};
        //     this.checkIfCloseToWater(plantData, mapManager);
    ...
    private removeSurroundingWater(): void {
        this.waterSources.forEach(pos => {
            ...
            if(this._mapManager.mapData.waterAmount[pos.y][pos.x] < 0){
                ...
                this._mapManager.mapData.waterData[pos.y][pos.x];
            }
        })
    }

    private updatePlayerWater(scene: Phaser.Scene,plantData: PlantData, waterStats: waterStats): void{
    ...
    private addWaterFromWaterSources( plantData: PlantData): number {
        let waterToAdd = 0;
    ...
    private removeWater(plantData: PlantData): number {
