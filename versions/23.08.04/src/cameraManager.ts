...
export default class CameraManager {
    ...
    constructor(scene: Phaser.Scene, plantManager: PlantManager, mapManager: MapManager){
        ...
        this.cam.setBounds(Game_Config.MAP_tilesToWorld(0), Game_Config.MAP_tilesToWorld(0), Game_Config.MAP_tilesToWorld(Game_Config.MAP_SIZE), Game_Config.MAP_tilesToWorld(Game_Config.MAP_SIZE));
        this.setupDragMovement(scene);

        this.maskTexture = scene.add.renderTexture(0, 0, Game_Config.MAP_SCALE*Game_Config.MAP_RES*Game_Config.MAP_SIZE, Game_Config.MAP_SCALE*Game_Config.MAP_RES*Game_Config.MAP_SIZE)
            .setOrigin(0,0)
            .setVisible(false);
        
        this.skyMask = scene.add.rectangle(0, 0, Game_Config.MAP_SIZE*Game_Config.MAP_RES*Game_Config.MAP_SCALE, (Game_Config.MAP_GROUND_LEVEL)*Game_Config.MAP_RES*Game_Config.MAP_SCALE, 0x000000)
            .setVisible(false)
            .setAlpha(0.95)
            .setOrigin(0);
    ...
    private updateMask(scene: Phaser.Scene, plantManager: PlantManager, mapManager: MapManager){
        ...
        for(let x = 1; x < Game_Config.MAP_SIZE - 1; x++){
            for(let y = 1; y < Game_Config.MAP_SIZE - 1; y++){
